	...
	const std::string wd)
{
	...
	// Load DATA statistics structure
	...
	const std::string wd)
{
	...
	data_evt_time evt_time;
	dmpch->GetEntry(0);
	evt_time.start_second = vars.second;
	evt_time.start_msecond = vars.msecond;
	dmpch->GetEntry(nevents -1);
	evt_time.end_second = vars.second;
	evt_time.end_msecond = vars.msecond;
	...
	if (verbose)
		std::cout << "Parsing skimmed data..." << std::endl;
	...
	for (unsigned int evIdx = 0; evIdx < nevents; ++evIdx)
	{
		...
		// Link BGO energy fraction layer
		vars.fracLayer = *vars.fracLayer_ptr;

		// Extract data time information
		h_second.Fill(vars.second);
		h_msecond.Fill(vars.msecond);

		// Event printout
		if (verbose)
			updateProcessStatus(evIdx, kStep, nevents);

		h_trigger.Fill(vars.energy * _GeV);		
		h_BGOrec_energy.Fill(vars.energy * _GeV);
		...
		extracted_psd_charge.chargeX = vars.PSD_chargeX;
		extracted_psd_charge.chargeY = vars.PSD_chargeY;
		extracted_stk_charge.chargeX = vars.STK_chargeX;
		extracted_stk_charge.chargeY = vars.STK_chargeY;
		
		// Fill cuts histos

		// Fill geometric cut histos
		if (vars.evtfilter_geometric)
		{
			h_geometric_cut.Fill(vars.energy * _GeV);

			// Evaluate the position on the First BGO layer (after geometric cut)
			evaluateTopBottomPosition_dataskimmed(
				vars.BGOrec_slopeX,
				vars.BGOrec_slopeY,
				vars.BGOrec_interceptX,
				vars.BGOrec_interceptY,
				h_geo_BGOrec_slopeX,
				h_geo_BGOrec_slopeY,
				h_geo_BGOrec_interceptX,
				h_geo_BGOrec_interceptY,
			...
			// Geometric cut && maxElayer cut
			if (vars.evtfilter_BGO_fiducial_maxElayer_cut)
				h_geometric_maxElayer_cut.Fill(vars.energy * _GeV);

			// Geometric cut && maxBarLayer cut
			if (vars.evtfilter_BGO_fiducial_maxBarLayer_cut)
				h_geometric_maxBarLayer_cut.Fill(vars.energy * _GeV);

			// Geometric cut && BGOTrackContainment cut
			if (vars.evtfilter_BGO_fiducial_BGOTrackContainment_cut)
				h_geometric_BGOTrackContainment_cut.Fill(vars.energy * _GeV);

			// Geometric cut && BGO fiducial cut
			if (vars.evtfilter_BGO_fiducial)
				h_geometric_BGO_fiducial_cut.Fill(vars.energy * _GeV);

			// Geometric cut and all cuts
			if (vars.evtfilter_all_cut)
			{
				h_geometric_all_cut.Fill(vars.energy * _GeV);
				h_geometric_all_cut_ce.Fill(vars.energy_corr * _GeV);
			}
		}

		// Fill BGO_fiducial_maxElayer cut histo
		if (vars.evtfilter_BGO_fiducial_maxElayer_cut)
			h_maxElayer_cut.Fill(vars.energy * _GeV);

		// Fill BGO_fiducial_maxBarLayer cut histo
		if (vars.evtfilter_BGO_fiducial_maxBarLayer_cut)
			h_maxBarLayer_cut.Fill(vars.energy * _GeV);

		// Fill BGO_fiducial_BGOTrackContainment cut histo
		if (vars.evtfilter_BGO_fiducial_BGOTrackContainment_cut)
			h_BGOTrackContainment_cut.Fill(vars.energy * _GeV);

		// Fill BGO fiducial volume cut
		if (vars.evtfilter_BGO_fiducial)
		{
			h_BGO_fiducial_cut.Fill(vars.energy * _GeV);

			// BGO fiducial cut && nBarLayer13 cut
			if (vars.evtfilter_nBarLayer13_cut)
				h_BGOfiducial_nBarLayer13_cut.Fill(vars.energy * _GeV);

			// BGO fiducial cut && maxRms cut
			if (vars.evtfilter_maxRms_cut)
				h_BGOfiducial_maxRms_cut.Fill(vars.energy * _GeV);

			// BGO fiducial cut && track selection cut
			if (vars.evtfilter_track_selection_cut)
				h_BGOfiducial_track_selection_cut.Fill(vars.energy * _GeV);
			
			// BGO fiducial cut && PSD-STK match cut
			if (vars.evtfilter_psd_stk_match_cut)
				h_BGOfiducial_psd_stk_match_cut.Fill(vars.energy * _GeV);
			
			// BGO fiducial cut && PSD charge cut
			if (vars.evtfilter_psd_charge_cut)
				h_BGOfiducial_psd_charge_cut.Fill(vars.energy * _GeV);

			// BGO fiducial cut && STK charge cut
			if (vars.evtfilter_stk_charge_cut)
				h_BGOfiducial_stk_charge_cut.Fill(vars.energy * _GeV);

			// BGO fiducial cut && XTRL cut
			if (vars.evtfilter_xtrl_cut)
				h_BGOfiducial_xtrl_cut.Fill(vars.energy * _GeV);

			// BGO fiducial cut && all cut
			if (vars.evtfilter_all_cut)
			{
				h_BGOfiducial_all_cut.Fill(vars.energy * _GeV);
				h_BGOfiducial_all_cut_ce.Fill(vars.energy_corr * _GeV);
			}
		}
		...
		if (vars.nActiveCuts)
		{
			if (vars.evtfilter_all_cut)
			{
				h_all_cut.Fill(vars.energy * _GeV);
				h_all_cut_ce.Fill(vars.energy_corr * _GeV);

				// Fill sumRms - cosine correlation histo
				fill_sumRms_cosine_histo(
					vars.sumRms,
					vars.STK_bestTrack_costheta,
					logEBins,
					sumRms_cosine,
					sumRms_cosine_20_100,
					sumRms_cosine_100_250,
			...
			if (vars.evtfilter_all_cut_no_xtrl)
			{
				// Fill xtrl histos
				fill_XTRL_histo(
					vars.sumRms,
