...
def buildHisto(opts: argparse.Namespace, files: list, pars: dict):
    ...
    plt.plot(np.array(stats['timebins']), np.array(stats['evts_20_100']), marker='o', label="20 - 100 GeV", color="cornflowerblue", linestyle='dashed', linewidth=2, markersize=2)
    plt.plot(np.array(stats['timebins']), np.array(stats['evts_100_250']), marker='^', label="100 - 250 GeV", color="darkorange", linestyle='dashed', linewidth=2, markersize=2)
    plt.plot(np.array(stats['timebins']), np.array(stats['evts_250_500']), marker='*', label="250 - 500 GeV", color="forestgreen", linestyle='dashed', linewidth=2, markersize=2)
    plt.plot(np.array(stats['timebins']), np.array(stats['evts_500_1']), marker='|', label="0.5 - 1 TeV", color="crimson", linestyle='dashed', linewidth=2, markersize=2)
    plt.plot(np.array(stats['timebins']), np.array(stats['evts_1_5']), marker='d', label="1 - 5 TeV", color="blueviolet", linestyle='dashed', linewidth=2, markersize=2)
    plt.plot(np.array(stats['timebins']), np.array(stats['evts_5']), marker='s', label="> 5 TeV", color="saddlebrown", linestyle='dashed', linewidth=2, markersize=2)
    '''
